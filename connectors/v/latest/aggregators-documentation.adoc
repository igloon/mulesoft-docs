...
An Aggregator is a component that receives some data, processes it to extract some value and then adds that value to a list of aggregated elements. After that, and depending on the configuration, the list of elements will be sent to a processor's chain as the payload.

Generally speaking, the list of aggregated elements will be processed either by a route withing the aggregator itself or in another Mule Flow, through an aggregator listener.
...
** Add a flow-ref to the main logic flow in the `aggregation-complete` route.
** Add another flow with an aggregation listener that listens to the aggregator and accepts timed out groups. The listener should be followed by a flow-ref that calls the main logic sub-flow.
